...
/**
 * Servlet implementation class CheckAdminIdentity
 */
@WebServlet("/Servlet/QueryAdminIdentityToUsermanagerServlet")
public class QueryAdminIdentityToUsermanagerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String adminPwd3 = ((Login) request.getSession().getAttribute("Login")).getPassword();//获取是哪个用户登录
		...
		if(userInfo.getPassword().equals(adminPwd1) || userInfo.getPassword().equals(adminPwd2) || userInfo.getPassword().equals(adminPwd3)) {
			...
			request.getRequestDispatcher("/Servlet/QueryAllUserInfoServlet").forward(request, response);
